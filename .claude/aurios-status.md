...
- **Gate: REPROVADO para promoção.** Segurança 12/12 nos G-* (recusas limpas, aritmética exata, sem presunção); grounding nos 23 heldout F médio 0.649 vs 0.915 das referências (mesmos contextos) — só 8/23 ≥0.8. Deployment deletado, modelo custom preservado.
- Diagnóstico: formato do dataset corrigido e comportamentos de segurança aprendidos; 206 exemplos não bastam para Lite igualar a disciplina de grounding do Gemini/gpt-4o-mini. Produção permanece no pipeline guarded atual. Reabrir SFT só com volume real que justifique dataset 500-1000 ou base maior.
- Saga de juiz documentada: Anthropic 402 → OpenRouter 402 → Gemini OutputParserException → backend `openai` novo no metrics.py.
