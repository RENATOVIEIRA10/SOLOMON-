...
/**
 * Every condition PDF paired with the canonical `source_url` already present in
 ...
 *
 * Generated by matching the crawler archive against the distinct
 * `conditions_pdf` URLs per insurer (1:1, no leftovers on either side). It
 * covers ONLY general conditions — MAG's sales guide, sustainability report,
 * e-book and operating procedure are excluded, even though the ingestion
 * pipeline mislabels them as `conditions_pdf`.
 */
...
const MANIFEST_PATH = path.join('scripts', 'phase2', 'odl-conditions-manifest.json')

function loadManifest(): readonly ManifestEntry[] {
  if (!existsSync(MANIFEST_PATH)) {
    throw new Error(`manifest not found at ${MANIFEST_PATH} (run from app/)`)
  }
  return JSON.parse(readFileSync(MANIFEST_PATH, 'utf8')) as ManifestEntry[]
}

const MANIFEST: readonly ManifestEntry[] = loadManifest()
...
function renderReport(results: RunResult[], wrote: boolean): void {
  ...
  // per-insurer aggregate — 42 documents is too many to read one by one
  const byInsurer = new Map<string, { docs: number; pages: number; rows: number; tables: number }>()
  for (const r of results) {
    const agg = byInsurer.get(r.insurer) ?? { docs: 0, pages: 0, rows: 0, tables: 0 }
    agg.docs++
    agg.pages += r.build.summary.pageCount
    agg.rows += r.build.rows.length
    agg.tables += r.tableChunks
    byInsurer.set(r.insurer, agg)
  }

  console.log('\n| insurer              | docs | pages | rows | TABLES |')
  console.log('|---|---|---|---|---|')
  for (const [insurer, a] of byInsurer) {
    console.log(
      `| ${insurer.padEnd(20)} | ${String(a.docs).padStart(4)} | ${String(a.pages).padStart(5)} | ` +
        `${String(a.rows).padStart(4)} | ${String(a.tables).padStart(6)} |`,
    )
  }
  ...
  const noTables = results.filter((r) => r.tableChunks === 0).length
  if (noTables > 0) console.log(`Note: ${noTables}/${results.length} documents yielded no table at all.`)
  ...
  if (wrote) {
    ...
    // one preview per insurer, biggest table wins — enough to eyeball structure
    const seen = new Set<string>()
    for (const r of [...results].sort((a, b) => (b.biggestTable?.length ?? 0) - (a.biggestTable?.length ?? 0))) {
      if (!r.biggestTable || seen.has(r.insurer)) continue
      seen.add(r.insurer)
      console.log(`\n--- ${r.insurer} / ${r.pdf}: biggest table (preview) ---`)
      console.log(r.biggestTable.split('\n').slice(0, 5).join('\n'))
    }
  }
}
