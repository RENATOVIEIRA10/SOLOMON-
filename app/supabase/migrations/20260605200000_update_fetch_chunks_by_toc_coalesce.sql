-- Migration: Update fetch_chunks_by_toc to use coalesce(d.metadata->>'source_doc', d.source_url)
-- Date: 2026-06-05
-- Context: PageIndex Lite (Roadmap Ciclo 003) support for documents identified by source_url
...
BEGIN
  ...
  JOIN matched_sections ms ON (coalesce(d.metadata->>'source_doc', d.source_url) = ms.source_doc)
  ...
  ORDER BY 
    coalesce(d.metadata->>'source_doc', d.source_url), 
    (
      CASE 
        WHEN d.metadata->>'page' ~ '^[0-9]+$' THEN (d.metadata->>'page')::integer
        ELSE -1
