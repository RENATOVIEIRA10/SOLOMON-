...
export interface SearchOptions {
  ...
  /**
   * Technical or formal insurance terms generated by LLM Query Expansion.
...
export async function lexicalSearch(
  ...
  const extracted = extractLexicalTerms(query, RAG.lexicalMaxTerms)
  const additional = (options?.expandedTerms ?? []).flatMap((term) =>
    extractLexicalTerms(term, 3)
  )
  const terms = [...new Set([...extracted, ...additional])].slice(0, RAG.lexicalMaxTerms * 2)
