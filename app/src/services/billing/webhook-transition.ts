...
export interface TransitionResult {
  update: { billing_status: string; overdue_since: string | null; billing_updated_at: string } | null
  notify: boolean
}

export function decideWebhookTransition(input: TransitionInput): TransitionResult {
  const { eventType, currentOverdueSince, nowISO } = input

  if (eventType === 'PAYMENT_CONFIRMED' || eventType === 'PAYMENT_RECEIVED') {
    return {
      update: { billing_status: 'active', overdue_since: null, billing_updated_at: nowISO },
      notify: false,
    }
  }
