...
import type { BrokerProfile } from "@/types/api";
...
export function ProfileView() {
  ...
  // Inicializa o form só quando o id do perfil muda (primeira carga ou save
  // bem-sucedido) — revalidação SWR em background não deve sobrescrever
  // edições em andamento do usuário.
  const loadedProfileId = useRef<string | null>(null);
  useEffect(() => {
    if (profile && profile.id !== loadedProfileId.current) {
  ...
  async function save(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const d = await apiFetch<{ profile: BrokerProfile }>("/api/profile", {
        method: "PUT",
      ...
      // Atualiza o cache SWR com o perfil salvo (sem revalidar de novo) —
      // corrige a staleness apontada no review final.
      mutate({ profile: d.profile }, { revalidate: false });
      setSaved(true);
